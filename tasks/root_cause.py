...
class RootCauseDiagnosisTask(BaseTask):
    ...
    def _resolve_query(self, query_value: str) -> List[str]:
        ...
        # Handle dynamic variants if present
        v = getattr(self, "scenario_variant", None)
        if v and "query_responses" in v:
            responses = v["query_responses"]
            for key, response in responses.items():
                if key.lower() in query_lower or query_lower in key.lower():
                    return [response]
            return ["No specific results found. Try querying: disk, wal, payment logs, errors."]
